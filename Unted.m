...
xlabel('time')
ylabel('Amplitude')
title('Frequency Response of hp[n]')
subplot(3,1,2)
plot(komega,abs(y3))
xlabel('\omega')
ylabel('|F(rect)(\omega)|')
...
xlabel('time')
ylabel('Amplitude')
title(' Frequency Response of msignal')
subplot(3,1,2)
plot(komega,abs(y4))
xlabel('\omega')
ylabel('|F(rect)(\omega)|')
...
figure(18)
stem(moutput)
kmout=0:(length(moutput)-1);
msin= sin(omega0*kmout).*moutput;

mcos= cos(omega0*kmout).*moutput;
mlc=conv(lp,msin);
mls=conv(lp,mcos);
dsignal=mls+mlc;
figure(28)
stem(dsignal)
%so for some reason it's upside down at at .5 magnnitude but we did it
%maybe??